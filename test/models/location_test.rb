...
class LocationTest < ActiveSupport::TestCase
  ...
  test "sets columns, header, updatable columns correctly" do
    assert_equal([:name, :address, :city, :state, :zip, :phone, :monkeys], Child.table_columns, "Columns set incorrectly")
    assert_equal(["Name", "Address", "City", "State", "Zip", "Phone", "Monkeys"], Child.table_headers, "Headers set incorrectly")
    ...
    assert_equal([:name, :address, :city, :state, :zip, :phone, :monkeys], Child.update_fields.map(&:name), "Updatable fields set incorrectly")
  end
  ...
  test "default scope restricts results" do
    child = Child.create(monkeys: 'bob')
